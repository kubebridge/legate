...
[<Fact>]
let ``Turn loop logs carry all six scopes and redact tool secrets`` () =
    let secret = "sk-ant-turn-secret-22222222"
    ...
    let invocations = ref []
    let fn = stubTool "lookup" $"output holding {secret} inside" invocations

    let client =
        scripted
            [
                callStep "call-1" "lookup"
                textStep "done"
            ]
